    ...
    (unwind-protect
        (progn
          ...
          (cl-letf (((symbol-function 'projectile-project-buffers-non-visible)
                    ...
                    ((symbol-function 'ghostel-toggle--view-for-root)
                     (lambda (&rest _) nil))
...
(ert-deftest projectile-config-switch-project-prefers-fullscreen-agent ()
  (let* ((root "/tmp/project/")
         (agent-buffer (generate-new-buffer " *project-agent-fullscreen*"))
         (text-buffer (generate-new-buffer " *project-text-fallback*"))
         (view (list :kind 'agent :root root :buffer agent-buffer
                     :dismiss nil :hidden t))
         shown-view)
    (unwind-protect
        (progn
          (with-current-buffer text-buffer
            (setq buffer-file-name "/tmp/project/notes.md"))
          (cl-letf (((symbol-function 'projectile-acquire-root)
                     (lambda () root))
                    ((symbol-function 'ghostel-toggle--view-for-root)
                     (lambda (kind seen-root)
                       (should (eq kind 'agent))
                       (should (equal seen-root root))
                       view))
                    ((symbol-function 'ghostel-toggle--show-fullscreen)
                     (lambda (seen-view) (setq shown-view seen-view)))
                    ((symbol-function 'projectile-project-buffers-non-visible)
                     (lambda () (list text-buffer)))
                    ((symbol-function 'switch-to-buffer)
                     (lambda (&rest _)
                       (ert-fail "Fullscreen agent should win"))))
            (my/projectile-switch-to-last-buffer-or-file))
          (should (eq shown-view view)))
      (kill-buffer agent-buffer)
      (kill-buffer text-buffer))))

(ert-deftest projectile-config-switch-project-skips-home-agent-view ()
  (let* ((root "/tmp/project/")
         (home-agent-buffer (generate-new-buffer " *home-agent-fullscreen*"))
         (text-buffer (generate-new-buffer " *project-text-after-home*"))
         (home-view (list :kind 'agent :root root :buffer home-agent-buffer
                          :dismiss t :hidden nil))
         switched-buffer)
    (unwind-protect
        (progn
          (with-current-buffer text-buffer
          ...
          (cl-letf (((symbol-function 'projectile-acquire-root)
                     (lambda () root))
                    ((symbol-function 'ghostel-toggle--view-for-root)
                     (lambda (&rest _) home-view))
                    ((symbol-function 'ghostel-toggle--show-fullscreen)
                     (lambda (&rest _)
                       (ert-fail "C-s-l home agent must be skipped")))
                    ((symbol-function 'projectile-project-buffers-non-visible)
                     (lambda () (list home-agent-buffer text-buffer)))
                    ((symbol-function 'switch-to-buffer)
                     (lambda (buffer &rest _)
                       (setq switched-buffer buffer))))
            (my/projectile-switch-to-last-buffer-or-file))
          (should (eq switched-buffer text-buffer)))
      (kill-buffer home-agent-buffer)
      (kill-buffer text-buffer))))
